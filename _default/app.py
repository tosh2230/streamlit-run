...
def get_session():
    ...
    if session_info is None:
        raise RuntimeError("Failed in getting Streamlit Session object.")
...
def show(df):
    ...
    if is_checked:
        ...
        selected_items = st.sidebar.multiselect(
            "Select columns", list(df.columns.values), selected_items
        )
        graph_height = st.sidebar.number_input(
            "Set graph height", 0, 1000, graph_height
        )
        graph_width = st.sidebar.number_input("Set graph width", 0, 1000, graph_width)
        histogram_mode = st.sidebar.selectbox(
            "Select a histogram mode", ["relative", "overlay"], index=0
        )
    ...
    with st.expander("Show details"):
        details_window_length = st.slider(
            "Set window size", 0, 500, details_window_length
        )
    ...
    line_chart = px.line(
        df_filtered, height=graph_height, width=graph_width, title="Line chart"
    )
    st.write(line_chart)
    ...
    histogram = px.histogram(
        ...
        title=f"Histogram: {histogram_mode}",
    )
